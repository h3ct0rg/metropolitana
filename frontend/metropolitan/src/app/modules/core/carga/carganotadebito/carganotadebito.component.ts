...
@Component({
  selector: 'app-carganotadebito',
  templateUrl: './carganotadebito.component.html',
  styleUrls: ['./carganotadebito.component.css']
})
export class CarganotadebitoComponent implements OnInit {
  ...
  constructor(
    ...
    private notaDebitoCalculateServices: CargaNotaDebitoCalculateService,
  ...
  private loadDropdowns() {
    ...
    this.operadoresService.getOperadorBySucursalCarga(this.getActualSucursal()).subscribe(data => {
  ...
  counterChange($event) {
    this.counterService.getCounterClientCarga($event).subscribe(result => {
      this.counters = result;
    });
  }
      ...
      if (this.isEdit) {
        ...
        this.notaDebitoServices.saveNotaDebitoItem(this.notaDev).subscribe(data => {
          ...
          this.ordenPagoService.saveOrdenPagoItem(this.localOrdenPago).subscribe(data => {
            ...
            this.logService.saveLogItemCarga(this.logs).subscribe(success => {
              this.router.navigate(['/main/carga/profile-nota-debito/' + idND]);
            });
          });
        });
      }
