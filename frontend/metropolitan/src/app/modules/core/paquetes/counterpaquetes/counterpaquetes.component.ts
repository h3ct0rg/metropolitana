...
@Component({
  selector: 'app-counterpaquetes',
  templateUrl: './counterpaquetes.component.html',
  styleUrls: ['./counterpaquetes.component.css']
})
export class CounterpaquetesComponent implements OnInit {
  ...
  ngOnInit() {
    this.route.paramMap.subscribe(param => {
      ...
      if (this.CounterId) {
        ...
        this.operadorService.getOperadorBySucursalPaquetes(this.getActualSucursal()).subscribe(resultPro => {
          let ll: TransferItem[] = [];
  ...
  loadFarmInformation(clienteId) {
    this.CounterService.getCounterPaquet(clienteId).subscribe((result) => {
  ...
  saveCounter() {
    ...
    if (this.isEdit) {
      ...
      this.CounterService.updateCounterPaquet(this.Counter).subscribe((data) => {
        this.router.navigate(['/main/paquetes/counterList']);
      });
    }
    else {
      this.CounterService.savetCounterItemPaquet(this.Counter).subscribe((data) => {
        this.router.navigate(['/main/paquetes/counterList']);
      });
    }
  }
