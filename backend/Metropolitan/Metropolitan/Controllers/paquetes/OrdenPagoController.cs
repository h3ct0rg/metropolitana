...
namespace Metropolitan.Controllers.paquetes
{
    ...
    [ApiController]
    public class PaquetesOrdenPagoController : ControllerBase
    {
        paquetesOrdendePagoTravelaceManagement gestordb = new paquetesOrdendePagoTravelaceManagement();
        ...
        [HttpPost("GetReporteVentasDetalleByCityFiltrada")]
        public ActionResult<List<ReporteOperacionPagoTravelaceDetalleFiltrado>> GetReporteVentasDetalleByCityFiltrada([FromBody] JObject actionRequest)
        {
            dynamic apRequest = (dynamic)actionRequest;
            var starDate = (string)apRequest.startDate;
            var endDate = (string)apRequest.endDate;
            ...
            var codOperador = (string)apRequest.codOperator;

            try
            {
                List<ReporteOperacionPagoTravelaceDetalleFiltrado> result = gestordb.getReportOrdenPagoDetalleByCityFiltrado(
                    Convert.ToDateTime(starDate), Convert.ToDateTime(endDate), idCity, voucher, codOperador);
                if (result.Count >= 0)
                {
                    return result;
                }
        ...
        [HttpPost("GetReporteVentasDetalleByCityFiltradaAnulado")]
        public ActionResult<List<ReporteOperacionPagoTravelaceDetalleFiltrado>> GetReporteVentasDetalleByCityFiltradaAnulado([FromBody] JObject actionRequest)
        {
            dynamic apRequest = (dynamic)actionRequest;
            var starDate = (string)apRequest.startDate;
            var endDate = (string)apRequest.endDate;
            ...
            {
                List<ReporteOperacionPagoTravelaceDetalleFiltrado> result = gestordb.getReportOrdenPagoDetalleByCityFiltrado(
                    Convert.ToDateTime(starDate), Convert.ToDateTime(endDate), idCity, voucher, codOperador,1);
                if (result.Count >= 0)
                {
                    return result;
                }
